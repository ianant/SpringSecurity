...
public class SpringSecurityConfigurationClass extends WebSecurityConfigurerAdapter {
	...
	protected void configure(AuthenticationManagerBuilder auth) throws Exception {
		 ...
		 * auth.inMemoryAuthentication().withUser(users.username("ianant").password(
		 * "test123").roles("employee","admin"));
		 * auth.inMemoryAuthentication().withUser(users.username("iashu").password(
		 * "test123").roles("employee"));
		 * auth.inMemoryAuthentication().withUser(users.username("imohit").password(
		 * "test123").roles("employee","manager"));
		 */	
		
		
		//read user through JDBC Authentication
